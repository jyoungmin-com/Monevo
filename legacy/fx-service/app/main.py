...
async def get_rates(base: str, quote: str):
    ...
    if cache_key in rates_cache:
        cached = rates_cache[cache_key]
        if current_time - cached["timestamp"] < 900:
            return cached["data"]
    ...
    rates_cache[cache_key] = {"data": result, "timestamp": current_time}
...
async def get_historical_rates(base: str, quote: str, date: str):
    ...
    if cache_key in rates_cache:
        return rates_cache[cache_key]["data"]
    ...
    rates_cache[cache_key] = {"data": result, "timestamp": 0}
